...
```js
// Avoid
User.query(function onSuccess(users) {
  throw new Error();
...
// Prefer
User.query()
  .$promise.then(function onSuccess(users) {
    throw new Error();
  })
...
_Custom_ parsers that fail to parse on input types "email", "url", "number", "date", "month",
...
`oldValues` represented the _most recent change of each entry_.
...
1. Passing an array or jqLite/jQuery collection of parent-less text nodes to `$compile` directly:
   ...
   let compiledNodes = angular.element(
...
2. Using multi-slot transclusion with non-consecutive, default-content text nodes (that form
   ...
   <!-- Before: -->
   <some-thing>
     <ng-transclude> {{ <-- Two separate 'foo'}} <-- text nodes </ng-transclude>
   </some-thing>

   <!-- After: -->
   <some-thing>
     <ng-transclude>
       foo <-- The text nodes were merged into `{{'foo'}}`, which was then
       interpolated
     </ng-transclude>
   </some-thing>
   ...
   <some-thing>
     <ng-transclude>
       <span>{{</span> <-- Two separate <span>'foo'}}</span> <-- nodes
     </ng-transclude>
   </some-thing>
   ```
...
describe("$q.when", function () {
  it("should not need a call to $timeout.flush() to resolve already resolved promises", inject(function (
    $q,
    $timeout,
  ) {
    $q.when("foo");
  ...
  it("should accept $timeout.flush(0) when not sure if $q.when was called with a value or a promise", inject(function (
    $q,
    $timeout,
  ) {
    $q.when("foo");
  ...
  it("should need a call to $timeout.flush() to resolve $q.when when called with a promise", inject(function (
    $q,
    $timeout,
  ) {
    $q.when($q.when("foo"));
...
- **filterFilter:** due to [a75537d4](https://github.com/angular/angular.js/commit/a75537d461c92e3455e372ff5005bf0cad2d2e95),
  ...
  **or deeper**). E.g.:
  ...
  arr = filterFilter([{ level1: { level2: "test" } }], {
    level1: { level2: "test" },
  });
...
- **Angular:** remove duplicate nodeName\_ references
...
- **nodeName\_:** simplify the code and reduce the number of DOM calls
...
```html
<div
  ng-attr-style="border-radius: {{value}}{{units}}"
...
- **$compile:** bind ng-attr-_ even if unbound attribute follows ng-attr-_
...
- **$parse:** mark constant unary minus expressions as constant
  ...
  [#6932](https://github.com/angular/angular.js/issues/6932))
- **Scope:**
  - revert the **proto** cleanup as that could cause regressions
...
- **Directives:**
  ...
  - **ngRepeat:** - handle iteration over identical obj values
    ...
    [#2806](https://github.com/angular/angular.js/issues/2806)) - support growing over multi-element groups
...
- rewrite of several major portions of angular.service._, angular.Array._, angular.Object.\* docs
