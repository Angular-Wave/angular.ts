describe("resource", () => {
  ...
  describe("basic usage", () => {
    ...
    describe("lookupDottedPath", () => {
      ...
      it("should get dotted paths", () => {
        expect(lookupDottedPath(data, "a")).toEqual({
          b: "foo",
          c: null,
          "@d": "d-foo",
        });
    ...
    it("should include a request body when calling custom method with hasBody is true", () => {
      ...
      $httpBackend
        .expect("CREATE", "/fooresource", instant)
        .respond({ fid: 42 });
    ...
    it("should not throw if response.data is the resource object", () => {
      ...
      $httpBackend
        .expect("POST", "/CreditCard/123", angular.toJson(data))
        .respond(cc);
    ...
    it("should support escaping colons in url template", () => {
      ...
      $httpBackend
        .expect("GET", "http://localhost:8080/Path/foo/:stillPath/bar")
        .respond();
      R.get({ a: "foo", b: "bar" });
    });
    ...
    it("should not encode @ in url params", () => {
      ...
      const R = $resource("/Path/:a");
      $httpBackend
        .expect("GET", "/Path/doh@fo%20o?!do%26h=g%3Da+h&:bar=$baz@1")
        .respond("{}");
    ...
    it("should encode array params", () => {
      const R = $resource("/Path/:a");
      $httpBackend
        .expect("GET", "/Path/doh&foo?bar=baz1&bar=baz2")
        .respond("{}");
    ...
    it("should support IPv6 URLs", () => {
      ...
      test(
        "http://[2620:0:861:ed1a::1]/:ed1a",
        { ed1a: "foo" },
        "http://[2620:0:861:ed1a::1]/foo",
      );
      test(
        "http://[2620:0:861:ed1a::1]/:ed1a",
        {},
      ...
      function test(templateUrl, params, actualUrl) {
        const R = $resource(templateUrl, null, null, {
          stripTrailingSlashes: false,
        });
    ...
    it("should support params in the `hostname` part of the URL", () => {
      ...
      test("http://:hostname/", { hostname: "foo.com" }, "http://foo.com/");
      test(
        "http://:l2Domain.:l1Domain",
        { l1Domain: "com", l2Domain: "bar" },
        "http://bar.com",
      );
      test(
        "http://:l2Domain.:l1Domain/",
    ...
    it("should encode & in query params unless in query param value", () => {
      ...
      $httpBackend
        .expect("GET", "/api/myapp/resource?from=bar%20%26%20blanks")
        .respond("{}");
    ...
    it("should build resource with default param", () => {
      $httpBackend
        .expect("GET", "/Order/123/Line/456.visa?minimum=0.05")
        .respond({ id: "abc" });
    ...
    it("should support @_property lookups with underscores", () => {
      $httpBackend
        .expect("GET", "/Order/123")
        .respond({ _id: { _key: "123" }, count: 0 });
      const LineItem = $resource("/Order/:_id", { _id: "@_id._key" });
      const item = LineItem.get({ _id: 123 });
      $httpBackend.flush();
      expect(item).toEqual({ _id: { _key: "123" }, count: 0 });
      $httpBackend
        .expect("POST", "/Order/123")
        .respond({ _id: { _key: "123" }, count: 1 });
      item.$save();
      $httpBackend.flush();
      expect(item).toEqual({ _id: { _key: "123" }, count: 1 });
    });
    ...
    it("should create resource", () => {
      $httpBackend
        .expect("POST", "/CreditCard", '{"name":"misko"}')
        .respond({ id: 123, name: "misko" });

      const cc = CreditCard.save({ name: "misko" }, callback);
      expect(cc).toEqual({ name: "misko" });
      expect(callback).not.toHaveBeenCalled();

      $httpBackend.flush();
      expect(cc).toEqual({ id: 123, name: "misko" });
    ...
    it("should read resource", () => {
      $httpBackend
        .expect("GET", "/CreditCard/123")
        .respond({ id: 123, number: "9876" });
      ...
      $httpBackend.flush();
      expect(cc).toEqual({ id: 123, number: "9876" });
      expect(callback.calls.mostRecent().args[0]).toEqual(cc);
      expect(callback.calls.mostRecent().args[1]()).toEqual(
        Object.create(null),
      );
    ...
    it("should read partial resource", () => {
      ...
      $httpBackend
        .expect("GET", "/CreditCard/123")
        .respond({ id: { key: 123 }, number: "9876" });
      cc.$get(callback);
      $httpBackend.flush();
      expect(callback.calls.mostRecent().args[0]).toEqual(cc);
      expect(callback.calls.mostRecent().args[1]()).toEqual(
    ...
    it("should update resource", () => {
      $httpBackend
        .expect("POST", "/CreditCard/123", '{"id":{"key":123},"name":"misko"}')
        .respond({ id: { key: 123 }, name: "rama" });

      const cc = CreditCard.save({ id: { key: 123 }, name: "misko" }, callback);
      expect(cc).toEqual({ id: { key: 123 }, name: "misko" });
      expect(callback).not.toHaveBeenCalled();
      $httpBackend.flush();
    });
    ...
    it("should query resource", () => {
      $httpBackend
        .expect("GET", "/CreditCard?key=value")
        .respond([{ id: 1 }, { id: 2 }]);
    ...
    it("should post charge verb", () => {
      $httpBackend
        .expect("POST", "/CreditCard/123!charge?amount=10", '{"auth":"abc"}')
        .respond({ success: "ok" });
    ...
    it("should post charge verb on instance", () => {
      $httpBackend
        .expect(
          "POST",
          "/CreditCard/123!charge?amount=10",
          '{"id":{"key":123},"name":"misko"}',
        )
        .respond({ success: "ok" });
    ...
    it("should patch a resource", () => {
      $httpBackend
        .expectPATCH("/CreditCard/123", '{"name":"igor"}')
        .respond({ id: 123, name: "rama" });

      const card = CreditCard.patch({ id: 123 }, { name: "igor" }, callback);

      expect(card).toEqual({ name: "igor" });
      expect(callback).not.toHaveBeenCalled();
      $httpBackend.flush();
      expect(callback).toHaveBeenCalled();
      expect(card).toEqual({ id: 123, name: "rama" });
    });

    it("should create on save", () => {
      $httpBackend
        .expect("POST", "/CreditCard", '{"name":"misko"}')
        .respond({ id: 123 }, { header1: "a" });
      ...
      $httpBackend.flush();
      expect(cc).toEqual({ id: 123 });
      expect(callback.calls.mostRecent().args[0]).toEqual(cc);
      expect(callback.calls.mostRecent().args[1]()).toEqual(
        extend(Object.create(null), { header1: "a" }),
      );
    });
    ...
    it("should support dynamic default parameters (global)", () => {
      ...
      $httpBackend
        .expect("GET", "/Person/students/fedor")
        .respond({ id: "fedor", email: "f@f.com" });

      const fedor = Person.get({ id: "fedor" });
      $httpBackend.flush();

      expect(fedor).toEqual({ id: "fedor", email: "f@f.com" });
    });
    ...
    it("should pass resource object to dynamic default parameters", () => {
      ...
      $httpBackend
        .expect("GET", "/Person/fedor")
        .respond({ id: "fedor", email: "f@f.com", count: 1 });

      const fedor = Person.get();
      $httpBackend.flush();

      expect(fedor).toEqual({ id: "fedor", email: "f@f.com", count: 1 });

      $httpBackend
        .expect("POST", "/Person/fedor2")
        .respond({ id: "fedor2", email: "f2@f.com", count: 2 });

      fedor.id = "fedor2";
      fedor.$save();
      $httpBackend.flush();

      expect(fedor).toEqual({ id: "fedor2", email: "f2@f.com", count: 2 });
    });
    ...
    describe("promise api", () => {
      ...
      describe("single resource", () => {
        ...
        it("should return promise from action method calls", () => {
          ...
          $httpBackend
            .expect("POST", "/CreditCard")
            .respond({ id: 1, number: "9" });

          cc.$save().then(callback);

          $httpBackend.flush();
          expect(callback).toHaveBeenCalled();
          expect(cc).toEqual({ id: 1, number: "9" });
        });
        ...
        it("should pass the same transformed value to success callbacks and to promises", () => {
          $httpBackend
            .expect("GET", "/CreditCard")
            .respond(200, { value: "original" });
      ...
      describe("responseInterceptor", () => {
        ...
        it("should allow per action responseError interceptor that gets full response", () => {
          ...
          $httpBackend
            .expect("GET", "/CreditCard")
            .respond(404, { ignored: "stuff" }, { foo: "bar" }, "Ack");
          CreditCard = $resource(
            "/CreditCard",
            {},
            {
    ...
    describe("success mode", () => {
      it("should call the success callback (as 1st argument) on 2xx responses", () => {
        ...
        let status;
        let statusText;
        const successCb = jasmine
          .createSpy("successCb")
          .and.callFake((d, h, s, t) => {
        ...
        $httpBackend
          .expect("GET", "/CreditCard")
          .respond(status, {}, headers, statusText);
        $httpBackend.flush();
    ...
    describe("failure mode", () => {
      ...
      it("should call the error callback if provided on non 2xx response", () => {
        $httpBackend
          .expect("GET", "/CreditCard/123")
          .respond(ERROR_CODE, ERROR_RESPONSE);
    ...
    it("should transform request/response", () => {
      ...
      $httpBackend
        .expect("POST", "/Person/123", { __id: 123 })
        .respond({ __id: 456 });
    ...
    describe("suffix parameter", () => {
      describe("query", () => {
        ...
        it("should work when query parameters are supplied", () => {
          $httpBackend
            .expect("GET", "/users.json?red=blue")
            .respond([{ id: 1, name: "user1" }]);
          const UserService = $resource("/users/:user_id.json", {
            user_id: "@id",
          });
        ...
        it("should work when query parameters are supplied and the format is a resource parameter", () => {
          $httpBackend
            .expect("GET", "/users.json?red=blue")
            .respond([{ id: 1, name: "user1" }]);
          const UserService = $resource("/users/:user_id.:format", {
            user_id: "@id",
            format: "json",
          });
          const user = UserService.query({ red: "blue" });
        ...
        it("should not convert string literals in array into Resource objects", () => {
          ...
          $httpBackend.flush();
          expect(strings).toEqual(["mary", "jane"]);
        });
        ...
        it("should not convert number literals in array into Resource objects", () => {
          ...
          $httpBackend.flush();
          expect(numbers).toEqual([213, 456]);
        });
      ...
      describe("get", () => {
        ...
        it("should work when an id and query parameters are supplied", () => {
          $httpBackend
            .expect("GET", "/users/1.json?red=blue")
            .respond({ id: 1, name: "user1" });
          const UserService = $resource("/users/:user_id.json", {
            user_id: "@id",
          });
      ...
      describe("save", () => {
        it("should append the suffix", () => {
          $httpBackend
            .expect("POST", "/users.json", '{"name":"user1"}')
            .respond({ id: 123, name: "user1" });
          const UserService = $resource("/users/:user_id.json", {
            user_id: "@id",
          });
          ...
          expect(callback).not.toHaveBeenCalled();
          $httpBackend.flush();
          expect(user).toEqual({ id: 123, name: "user1" });
          expect(callback).toHaveBeenCalled();
        ...
        it("should append when an id is supplied", () => {
          $httpBackend
            .expect("POST", "/users/123.json", '{"id":123,"name":"newName"}')
            .respond({ id: 123, name: "newName" });
          const UserService = $resource("/users/:user_id.json", {
            user_id: "@id",
          });
          ...
          expect(callback).not.toHaveBeenCalled();
          $httpBackend.flush();
          expect(user).toEqual({ id: 123, name: "newName" });
          expect(callback).toHaveBeenCalled();
          expect(callback.calls.mostRecent().args[0]).toEqual(user);
          expect(callback.calls.mostRecent().args[1]()).toEqual(
            Object.create(null),
    ...
    describe("action-level url override", () => {
      it("should support overriding url template with static url", () => {
        $httpBackend
          .expect("GET", "/override-url?type=Customer&typeId=123")
          .respond({ id: "abc" });
        const TypeItem = $resource(
          "/:type/:typeId",
          { type: "Order" },
  ...
  describe("extra params", () => {
    ...
    beforeEach(
      module(($provide) => {
        $provide.decorator("$http", ($delegate) =>
          jasmine.createSpy("$http").and.callFake($delegate),
        );
      }),
    );
    ...
    it("should pass extra params to `$http` as `config.params`", () => {
      ...
      expect($http).toHaveBeenCalledWith(
        jasmine.objectContaining({ params: { baz: "qux" } }),
      );
    });
  ...
  describe("errors", () => {
    ...
    it("should fail if action expects an object but response is an array", () => {
      ...
      $resource("/Customer/123")
        .query()
        .$promise.then(successSpy, (e) => {
          failureSpy(e.message);
        });
      $httpBackend.flush();

      expect(successSpy).not.toHaveBeenCalled();
      expect(failureSpy).toHaveBeenCalled();
      expect(failureSpy.calls.mostRecent().args[0]).toEqualMinErr(
        "$resource",
        "badcfg",
  ...
  describe("handling rejections", () => {
    ...
    it("should report a PUR when the responseError interceptor returns a rejected promise", inject((
      $q,
    ) => {
      const CreditCard = $resource(
        "/CreditCard",
    ...
    it("should not propagate exceptions in success callback to the returned promise", () => {
      const successCallbackSpy = jasmine
        .createSpy("successCallback")
      ...
      $httpBackend.expectGET("/CreditCard/123").respond(null);
      const CreditCard = $resource("/CreditCard/:id");
      CreditCard.get({ id: 123 }, successCallbackSpy).$promise.then(
        promiseResolveSpy,
        promiseRejectSpy,
      );
    ...
    it("should not be able to recover from inside the error callback", () => {
      const errorCallbackSpy = jasmine
      ...
      const CreditCard = $resource("/CreditCard/:id");
      CreditCard.get({ id: 123 }, () => {}, errorCallbackSpy).$promise.then(
        promiseResolveSpy,
        promiseRejectSpy,
      );
  ...
  describe("cancelling requests", () => {
    ...
    it("should delete non-numeric timeouts in actions and log a $debug message", inject((
      $log,
      $q,
    ) => {
      spyOn($log, "debug");
      ...
      expect($log.debug).toHaveBeenCalledOnceWith(
        "ngResource:\n" +
    ...
    it("should use `cancellable` value if passed a non-numeric `timeout` in an action", inject((
      $log,
      $q,
      $rootScope,
    ) => {
    ...
    it("should accept numeric timeouts in cancellable actions and cancel the request when timeout occurs", () => {
      ...
      expect($httpBackend.flush).toThrow(
        new Error("No pending request to flush !"),
      );
  ...
  describe("configuring `cancellable` on the provider", () => {
    ...
    beforeEach(
      module("ngResource", ($resourceProvider) => {
