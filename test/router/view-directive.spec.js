...
describe("uiView", function () {
  ...
  describe("linking ui-directive", function () {
    it("anonymous ui-view should be replaced with the template of the current $state", inject(function (
      $state,
      $q,
    ) {
    ...
    it("named ui-view should be replaced with the template of the current $state", inject(function (
      $state,
      $q,
    ) {
      elem.append(
    ...
    it("ui-view should be updated after transition to another state", inject(function (
      $state,
      $q,
    ) {
      elem.append($compile("<div><ui-view></ui-view></div>")(scope));
    ...
    it("should handle nested ui-views (testing two levels deep)", inject(function (
      $state,
      $q,
    ) {
  ...
  describe("handling initial view", function () {
    it("initial view should be compiled if the view is empty", inject(function (
      $state,
      $q,
    ) {
    ...
    it("initial view should be put back after removal of the view", inject(function (
      $state,
      $q,
    ) {
      const content = "inner content";
    ...
    it("initial view should be transcluded once to prevent breaking other directives", inject(function (
      $state,
      $q,
    ) {
  ...
  describe("autoscroll attribute", function () {
    it("should NOT autoscroll when unspecified", inject(function (
      $state,
      $q,
      $uiViewScroll,
      $animate,
    ...
    it("should autoscroll when expression is missing", inject(function (
      $state,
      $q,
      $uiViewScroll,
      $animate,
    ...
    it("should autoscroll based on expression", inject(function (
      $state,
      $q,
      $uiViewScroll,
      $animate,
  ...
  it("should instantiate a controller with controllerAs", inject(function (
    $state,
    $q,
  ) {
    elem.append($compile("<div><ui-view></ui-view></div>")(scope));
  ...
  it("should instantiate a controller with both $scope and $element injections", inject(function (
    $state,
    $q,
  ) {
    elem.append(
  ...
  describe("(resolved data)", function () {
    ...
    it("should provide the resolved data on the $scope", inject(function (
      $state,
      $q,
      $timeout,
    ) {
    ...
    it("should put the resolved data on the resolveAs variable", inject(function (
      $state,
      $q,
      $timeout,
    ) {
    ...
    it("should put the resolved data on the controllerAs", inject(function (
      $state,
      $q,
      $timeout,
    ) {
  ...
  it("should call the existing $onInit after instantiating a controller", inject(function (
    $state,
    $q,
  ) {
  ...
  it("should default the template to a <ui-view>", inject(function (
    $state,
    $q,
  ) {
  ...
  describe("play nicely with other directives", function () {
    ...
    describe("working with ngRepeat", function () {
      ...
      it("should have correct number of uiViews", inject(function (
        $state,
        $q,
        $compile,
      ) {
      ...
      it("should populate each view with content", inject(function (
        $state,
        $q,
        $compile,
      ) {
      ...
      it("should interpolate ui-view names", inject(function (
        $state,
        $q,
        $compile,
      ) {
  ...
  describe("AngularJS Animations", function () {
    it("should do transition animations", inject(function (
      $state,
      $q,
      $compile,
      $animate,
    ...
    it("should do ngClass animations", inject(function (
      $state,
      $q,
      $compile,
      $animate,
    ...
    it("should do ngIf animations", inject(function (
      $state,
      $q,
      $compile,
      $animate,
...
describe("UiView", function () {
  ...
  it("shouldn't puke on weird nested view setups", inject(function (
    $compile,
    $rootScope,
    $q,
    $state,
...
describe("angular 1.5+ style .component()", function () {
  ...
  describe("routing using component templates", function () {
    ...
    it("should work with directives which themselves have templateUrls", function () {
      ...
      expect(
        angular.element(directiveEl).data("$ng12DirectiveController"),
      ).toBeDefined();
    ...
    if (angular.version.minor >= 3) {
      it("should work with ng 1.3+ bindToController directives", function () {
        ...
        expect(
          angular.element(directiveEl).data("$ng13DirectiveController"),
        ).toBeDefined();
        expect(el.text()).toBe("x-DATA!-x");
      });
    }
    ...
    if (angular.version.minor >= 5) {
      it("should work with ng 1.5+ .component()s", function () {
        ...
        expect(
          angular.element(directiveEl).data("$ngComponentController"),
        ).toBeDefined();
        expect(el.text()).toBe("x-DATA!-x");
      });
    }
  ...
  describe("+ component: declaration", function () {
    it("should disallow controller/template configuration", function () {
      ...
      expect(function () {
        $stateProvider.state(
          "route2cmp",
          extend({ templateProvider: function () {} }, stateDef),
        );
      }).toThrow();
      expect(function () {
        $stateProvider.state(
      ...
      expect(function () {
        $stateProvider.state(
          "route2cmp",
          extend({ controllerProvider: function () {} }, stateDef),
        );
      }).toThrow();
