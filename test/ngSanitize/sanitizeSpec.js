describe("HTML", () => {
  ...
  describe("htmlParser", () => {
    ...
    it('should not treat "<" followed by a non-/ or non-letter as a tag', () => {
      expectHTML("<- text1 text2 <1 text1 text2 <{", handler).toBe(
        "&lt;- text1 text2 &lt;1 text1 text2 &lt;{",
      );
    });
  ...
  it("should echo html", () => {
    expectHTML('hello<b class="1\'23" align=\'""\'>world</b>.').toBe(
  ...
  it("should escape non-start tags", () => {
    expectHTML("a< SCRIPT >A< SCRIPT >evil< / scrIpt >B< / scrIpt >c.").toBe(
  ...
  it("should remove double nested script", () => {
    expectHTML("a<SCRIPT>ev<script>evil</sCript>il</scrIpt>c.").toEqual(
      "ailc.",
    );
  });
  ...
  it("should remove unsafe value", () => {
    ...
    expectHTML('<img src="foo.gif" usemap="#foomap">').toEqual(
      '<img src="foo.gif">',
    );
  });
  ...
  it("should mangle improper html", () => {
    ...
    expectHTML('< div rel="</div>" alt=abc dir=\'"\' >text< /div>').toBe(
  ...
  it("should mangle improper html2", () => {
    ...
    expectHTML('< div rel="</div>" / >').toBe("&lt; div rel=&#34;&#34; / &gt;");
  });

  it("should ignore back slash as escape", () => {
    expectHTML('<img alt="xxx\\" title="><script>....">').toBe(
  ...
  it("should ignore object attributes", () => {
    expectHTML('<a constructor="hola">:)</a>').toEqual("<a>:)</a>");
    expectHTML('<constructor constructor="hola">:)</constructor>').toEqual("");
  });
  ...
  it('should accept tag delimiters such as "<" inside real tags (with nesting)', () => {
    ...
    expectHTML("<p> 10 < <span>100</span> </p>").toEqual(
      "<p> 10 &lt; <span>100</span> </p>",
    );
  });
  ...
  it("should prevent mXSS attacks", () => {
    expectHTML('<a href="&#x3000;javascript:alert(1)">CLICKME</a>').toBe(
      "<a>CLICKME</a>",
    );
  });
  ...
  it("should strip html comments", () => {
    expectHTML(
      "<!-- comment 1 --><p>text1<!-- comment 2 -->text2</p><!-- comment 3 -->",
  ...
  describe("clobbered elements", () => {
    it('should throw on a form with an input named "parentNode"', () => {
      inject(($sanitize) => {
        ...
        }).toThrow("$sanitize", "elclob");

        expect(() => {
          $sanitize(
            '<form><div><div><input name="parentNode" /></div></div></form>',
          );
        }).toThrow("$sanitize", "elclob");
      });
    });
    ...
    if (!/Edge\/\d{2,}/.test(window.navigator.userAgent)) {
      ...
      it('should throw on a form with an input named "nextSibling"', () => {
        inject(($sanitize) => {
          ...
          expect(() => {
            $sanitize(
              '<form><div><div><input name="nextSibling" /></div></div></form>',
            );
          }).toThrow("$sanitize", "elclob");
        });
      });
  ...
  it("should not allow JavaScript execution when creating inert document", inject((
    $sanitize,
  ) => {
  ...
  it("should not allow JavaScript hidden in badly formed HTML to get through sanitization (Firefox bug)", inject((
    $sanitize,
  ) => {
    const doc = $sanitize(
  ...
  describe("Custom white-list support", () => {
    ...
    it("should allow custom white-listed element", () => {
      ...
      expectHTML("<foo-button></foo-button>").toEqual(
    ...
    it("should not allow add custom element after service has been instantiated", inject((
      $sanitize,
    ) => {
  ...
  describe("SVG support", () => {
    ...
    it("should not ignore white-listed svg camelCased attributes", () => {
      expectHTML('<svg preserveAspectRatio="true"></svg>').toBe(
        '<svg preserveAspectRatio="true"></svg>',
        '<svg preserveAspectRatio="true" xmlns="http://www.w3.org/2000/svg"></svg>',
      );
    });

    it("should allow custom white-listed SVG element", () => {
      expectHTML("<font-face-uri></font-face-uri>").toEqual(
    ...
    it("should sanitize SVG xlink:href attribute values", () => {
      expectHTML(
        ...
        '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"><a></a></svg>',
    ...
    it("should sanitize SVG xml:base attribute values", () => {
      expectHTML(
        ...
        '<svg xmlns="http://www.w3.org/2000/svg"><a href="#"></a></svg>',
      );
    ...
    it("should not accept SVG animation tags", () => {
      expectHTML(
        ...
        '<svg xmlns:xlink="http://www.w3.org/1999/xlink"><a><text y="1em">Click me</text></a></svg>',
      ...
      expectHTML(
        ...
        '<svg><a xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="?"><circle r="400"></circle></a></svg>',
    ...
    it("should not accept SVG `use` tags", () => {
      expectHTML('<svg><use xlink:href="test.svg#xss" /></svg>').toBe(
        "<svg></svg>",
  ...
  describe("uri checking", () => {
    ...
    describe("javascript URLs", () => {
      ...
      it("should ignore hex encoded javascript:", () => {
        ...
        expect(
          "&#x6A;&#x61&#x76&#x61&#x73&#x63&#x72&#x69&#x70&#x74&#x3A;",
        ).not.toBeValidUrl();
        expect(
          "&#x6A &#x61&#x76&#x61&#x73&#x63&#x72&#x69&#x70&#x74&#x3A;",
        ).not.toBeValidUrl();
  ...
  describe("sanitizeText", () => {
    ...
    it("should escape text", () => {
      expect(sanitizeText("a<div>&</div>c")).toEqual(
        "a&lt;div&gt;&amp;&lt;/div&gt;c",
      );
    });
  });
